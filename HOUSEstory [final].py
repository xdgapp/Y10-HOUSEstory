...
def title():		#title screen of the entire game, player selects difficulty
	global userInput, easy, asbestosEnding, fallEnding, sisterEnding, dadEnding, good, true, trueTrue
	asbestosEnding = 0			#reset ending count
	fallEnding = 0
	sisterEnding = 0
	dadEnding = 0
	good = 0
	...
	print("------------------------------------")
	print("Welcome to...")
	print("  _   _  ___  _   _ ____  _____     _                   		  ____||____")			#ascii house by Veronica Karlsson (found online)
	...
	print("[standard] mode (recommended game mode, especially for first-time players), or")
	print("[easy] mode (standard mode but much easier, only play if you really struggle with standard mode)")
	print("(if input lines are empty and don't contain '> ' press the enter key to continue.)")         #suggested by Ashton C
	print("------------------------------------")
	userInput = ""
	while userInput not in options:
		...
		if userInput == "standard":
			...
			print("Input [standard] or [easy]")

def start():		#introduction and instructions
	global easy, showered, sisterKilled, blocked, mumBlinded, rugPulled, trapdoorLocked, crawlspaceDark, dadBlinded, dadBlinded, frontDoorLocked, flashlight, key, keys, skull, sistersBody, parentsRoomHint
	showered = 0							#reset conditions
	sisterKilled = 0
	blocked = 1
	mumBlinded = 0
	rugPulled = 0
	...
	frontDoorLocked = 1
	parentsRoomHint = 0
	flashlight = 0							#reset inventory and items
	key = 0
	keys = 0
	skull = 0
	sistersBody = 0
	print("------------------------------------")
	input("You wake up in your room, and according to your clock, it is 9:09 on a Saturday morning. The next thing you realised is that you were grounded by your parents the night before for hiding your sister's prized possession: her flashlight.")
	print()
	input("You get a text on your phone from your friends, inviting you to go out. You've got to find a way to escape the house.")
	print()
	print("Commands are indicated inside (and not including) square brackets, for example [take sword] is inputted as (without the quotation marks) 'take sword'.")
	print("Commands will only register and are expected if the input line begins with \">\", else just press enter key to reveal more text.")
	print("To look around the room, input [look].")
	print("To look at your inventory, input [inventory].")
	if easy == 0:
		print("To (try to) use an item in your inventory, input [use item] (replacing 'item' with the item name, e.g. [use flashlight])")
	input("Good luck.")
	bedroom()

def bedroom():			#starting room
	global userInput
	...
	userInput = ""
	while userInput not in options:
		userInput = input("> ").lower()				#suggested by Shafye M
		if userInput == "look":
			bedroom()
...
def bathroom():			#could shower in this room
	...
	while userInput not in options:
		...
		if userInput == "look":
			...
			if showered == 0:
				...
				print("(You are still in the same room, awaiting next input)")			#suggested by Kyan M
				print("------------------------------------")
				userInput = ""
			elif showered == 1:
				print("------------------------------------")
				print("You hate taking a shower, and you just showered. Are you some masochist?")
				print("(You are still in the same room, awaiting next input)")
				print("------------------------------------")
				userInput = ""
		elif userInput == "inventory":
			inventory()
...
def hallway():			#connects rooms to each other
	global userInput
...
def guestRoom():			#death trap room, essentially useless, but sort of fun and impactful
	global userInput, asbestosEnding
	...
	while userInput not in options:
		userInput = input("> ").lower()
		if userInput == "look":
			asbestosEnding = 1			#ending if the player stays in the guest room for too long
			print("------------------------------------")
			print("The plainest room you've ever encountered, white walls, white bed, white door. Only things of any interest is the wardrobe in the corner (which you can [examine]), and a wide open window you can [climb out] of.")
			print("To your [south], the hallway.")
			print("You still don't understand why this room has never been used. Wait, what's that smell?")
			print("------------------------------------")
			input("Suddenly, you become dizzy and discombobulated. \"Wait, that smell... It's asbestos isn't it?\" You drop to the floor, unconscious.")
			print("------------------------------------")
			...
			asbestosEnding = 1			#ending if the player stays in the guest room for too long
			print("------------------------------------")
			input("Empty. Makes a lot of sense. Suddenly, you become dizzy and discombobulated. \"Wait, that smell... It's asbestos isn't it?\" You drop to the floor, unconscious.")
			print("------------------------------------")
			print("(YOU HAVE UNLOCKED THE ASBESTOS ENDING, AKA THE DEATH BY ROOM ENDING, 1 OF 6 POSSIBLE ENDINGS. WOULD YOU LIKE TO PLAY AGAIN? yes/no)")
			userInput = ""
			playAgain()
...
def outTheWindow():				#climbing out of the guest room window always kills the player
	global userInput, fallEnding
	fallEnding = 1			#ending if the player climbed out of the two-storey house
	print("------------------------------------")
	input("As you try to fit your body through the window, you become disorientated, and you foolishly fall down the second storey of your house, headfirst.")
	print("------------------------------------")
...
def sistersRoom():			#sister is still alive, 'boss battle' with her
	global userInput, sisterKilled, sisterEnding
	print("------------------------------------")
	...
	print("------------------------------------")
	if showered == 1:
		sisterEnding = 1		#ending if the player showered before going into the sister's room
		input("With nothing at your disposal to fend for yourself, you stand there frozen in the middle of the room, and so your sister viciously tears apart your limbs, both legs at once with excruciating pain.")
		print()
		input("As you bring your arms to your head, your sister detaches both arms, leaving your head exposed. Bleeding out, you plead to your sister in a last attempt of survival. She reaches out her hand. She moves it closer to your head.")
		print()
		input("Then- SPLAT. With her bare hand, your sister crushes your skull.")
		print("------------------------------------")
		...
		sisterKilled = 1
		input("With nothing at your disposal to fend for yourself, you stand there frozen in the middle of the room. Your sister lets out an ear-piercing scream.")
		print()
		input("You know that you are done for, wait, hold on, the scream is no longer a scream. The yell that launched out of your sister's throat malformed into a seemingly endless stream of vomit. Your sister falls to the ground, faceup to the ceiling, and the next thing you hear is violent choking noises. You didn't shower.")
		print()
		input("No longer frozen, you walk up to your sister's body and you look down on her. She has died. Better her than you. You begin to [look] around the room.")
		deadSistersRoom()

def deadSistersRoom():			#sister now dead, more options as she doesn't stand guard anymore
	...
	while userInput not in options:
		...
		if userInput == "look":
			...
			if flashlight == 0:
				...
				print("------------------------------------")
				userInput = ""
		elif userInput == "lift":		#skull gives player the ability to lift anything
			if skull == 1:
				sistersBody = 1
				print("------------------------------------")
...
def livingRoom():			#similar to hallway, trapdoor needs to be discovered and unlocked to enter
	global userInput, easy, rugPulled, trapdoorLocked
	options = ["look", "south", "west", "upstairs", "downstairs", "pull", "use the key", "use key" "inventory"]
	print("------------------------------------")
...
def blockedParentsRoom():       #technically the door of the parents room, occurs before the player could enter the room
	global userInput, easy, blocked, mumBlinded, parentsRoomHint
	options = ["use flashlight on mum", "use flashlight"]
	print("------------------------------------")
	...
	if flashlight == 1:
		if easy == 1:
			...
			print("As your mum talks, you realise the flashlight she mentioned was in your hands at that very moment.")
			if parentsRoomHint >= 2:
					print("------------------------------------")
					print("(Remember, you could use items in your inventory by inputting with the [use] command.)")
		print("------------------------------------")
	else:
	...
	while userInput not in options:
		...
		if userInput == "use flashlight on mum":
			if easy == 1:
				...
				mumBlinded = 1
				print("------------------------------------")
				input("You quickly turn on and turn off the flashlight at your poor mother's eyes, effectively blinding her. \"AAGGHHHH!\" she says as she stumbles and finally falls on to the king-sized bed. You begin to look around the room.")
				parentsRoom()
			else:
				sorry()
				print("------------------------------------")
				input("You return to the living room...")
				livingRoom()
		if userInput == "use flashlight":
			if easy == 0:
				blocked = 0
				mumBlinded = 1
				print("------------------------------------")
				...
				parentsRoomHint += 1
				if parentsRoomHint >= 2:        #gives the player a hint on what to do with flashlight, suggested by Felix P and Kyan T
					print("------------------------------------")
					print("Remember, you could use items in your inventory by inputting with the [use] command.")
				sorry()
				print("------------------------------------")
...
def parentsRoom():        #player discovers old key to unlock trapdoor
...
def crawlspace():       #secret underground area, player finds house keys in here
	global userInput, easy, crawlspaceDark, flashlight, keys
	...
	if crawlspaceDark == 0:
		...
		print("------------------------------------")
		if easy == 1:
			print("You cannot see anything. You should [use flashlight]. [upstairs], the living room.")
		else:
			print("You cannot see anything. [upstairs], the living room.")
		print("------------------------------------")
	userInput = ""
	while userInput not in options:
		userInput = input("> ").lower()
		if userInput == "look":
			...
			if flashlight == 1:
				crawlspaceDark = 0
				print("------------------------------------")
				input("You can now see some things.")
				crawlspace()
			else:
				sorry()
				crawlspace()
			...
			print("------------------------------------")
			input("Before you leave the dark underground you turn your flashlight back off to conserve power.")
			crawlspaceDark = 1
			livingRoom()
		elif userInput == "inventory":
			inventory()
...
def skullRoom():		#taking the skull dictates whether escaping gives the good ending, or the true ending
...
def kitchen():			#final 'boss battle' with dad
	global userInput, easy, dadBlinded, frontDoorLocked, keys, flashlight, mumBlinded, showered, dadEnding
	options = ["look", "east", "west", "use keys" "use flashlight on dad", "use flashlight", "inventory"]
	print("------------------------------------")
	print("THE KITCHEN")
	print("------------------------------------")
	...
	while userInput not in options:
		...
		if userInput == "look":
			...
			if keys == 1 and dadBlinded == 1:
				if frontDoorLocked == 0 or easy == 1:
					if skull == 1:
						trueEnding()
					else:
						goodEnding()
				...
				if flashlight == 1 and mumBlinded == 1:
					if easy == 1:
						...
						print("However, with the flashlight in your hand, you wonder if that thing that you did with your mum will work with your dad...")
				print("(You are still in the same room, awaiting next input)")
				print("------------------------------------")
				userInput = ""
			else:
			...
			if easy == 1:
				if flashlight == 1:
					...
					if showered == 1:
						...
						dadEnding = 1
						print("------------------------------------")
						input("\"OH, YOU BLASTED-\" Your dad starts stumbling, but quickly reorients himself. \"Ohohoh, you dare try to defy my authority? You idiot of a son, I can smell you from all the way over here. ")
						print()
						input("Your blinded dad begins to charge towards you with full strength. You couldn't dodge quick enough to escape your dad's gigantic frame. He crushes you into the wall with no thought.")
						print("------------------------------------")
				...
				sorry()
				kitchen()
		elif userInput == "use keys":
			if easy == 0:
				if keys == 1:
					frontDoorLocked = 0
					print("------------------------------------")
					print("The front door is now unlocked.")
					print("(You are still in the same room, awaiting next input)")
					print("------------------------------------")
					userInput = ""
				else:
...
def goodEnding():		#ending if the player escaped and didn't take the skull
	global userInput, good
	good = 1
	print("------------------------------------")
	print("OUTSIDE(!)")
	print("------------------------------------")
	input("You did it, you finally made it out of the house, now you can meetup with your friends. Hopefully your sister didn't mind getting killed.")
	print()
	input("As you walk towards the bus stop, you update your friends on your status and tell them you're on your way.")
	print("------------------------------------")
...
def trueEnding():		#ending if the player escaped and took the skull
	...
	print("------------------------------------")
	input("All of a sudden, a hole is punched into an imaginary wall in front of you, on the other side is a slender, dark figure in a space filled with white light. A voice from this hole booms at you, \"YOU THERE. YOU DID THIS. YOU FAILED TO GUARD THE JERRICRANIUM.\"")
	print()
	print("You cannot believe it. The world may possibly be ending, and it may be your fault.")
	input("\"NOW WE SHALL REQUIRE A HUMAN BODY TO REPLACE IT. YOU WILL DO.\"")
	if sistersBody == 1:
		trueTrueEnding()
	else:
		print("------------------------------------")
		print("(YOU HAVE UNLOCKED THE TRUE ENDING, AKA THE HELL ON EARTH ENDING, 1 OF (NOW) 7 POSSIBLE ENDINGS (YES, THERE IS A 7th ENDING). WOULD YOU LIKE TO PLAY AGAIN? yes/no)")
		userInput = ""
		playAgain()

def trueTrueEnding():			#ending if the player escaped and took the skull AND took the sister's corpse
	global userInput, trueTrue
	trueTrue = 1
	...
	input("Might as well, it's nice outside.")
	print("------------------------------------")
...
def playAgain():		#asks player if they want to play again
	global userInput, asbestosEnding, fallEnding, sisterEnding, dadEnding, good, true, trueTrue, endings
	endings = int(asbestosEnding) + int(fallEnding) + int(sisterEnding) + int(dadEnding) + int(good) + int(true) + int(trueTrue)
	print("------------------------------------")
	print("(In this play session, you have unlocked " + str(endings) + " out of 6 endings.)")
	print("------------------------------------")
	userInput = ""
...
def inventory():		#shows the player what items they have
	...
	if flashlight == 1:
		...
		print("Nothing :(")
	if easy == 0:
		print("(remember that to use an item, you input [use item])")
	input()

def sorry():		#shown to player if an invalid command is inputted
	global userInput
	print("------------------------------------")
	if "use" in str(userInput):
		if easy == 0:
			input("Sorry, that item cannot be used right now (you may not have formatted your input properly, keep it simple like [use flashlight])")
		else:
			input("Sorry, that item cannot be used right now (check the given commands)")
	else:
		input("Sorry, I don't understand (commands are indicated with square brackets, [])")
	userInput = ""
